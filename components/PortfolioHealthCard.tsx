...
const CircularProgress = ({ score, size, color, trackColor, textColor, grade, isPrivacyMode }: any) => {
    const r = size * 0.4;
    const center = size / 2;
    ...
    return (
        <View style={{ alignItems: 'center', justifyContent: 'center', width: size, height: size }}>
            ...
            <View style={styles.compactScoreBox}>
                <Text style={[styles.compactScoreNum, { color }]}>{isPrivacyMode ? '**' : score}</Text>
                <Text style={[styles.compactScoreLabel, { color: textColor }]}>HEALTH</Text>
            </View>
        </View>
    );
};
...
// ─── Component ───────────────────────────────────────────────────────────────
export const PortfolioHealthCard = ({ isCompact = false }: { isCompact?: boolean }) => {
    ...
    return (
        <TouchableOpacity
            ...
            style={[
                ...
                isCompact && { flex: 1, padding: 16, marginBottom: 0, borderRadius: 24, justifyContent: 'space-between' }
            ]}
        >
            {/* Header */}
            <View style={styles.header}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    ...
                    <Text style={[styles.sectionLabel, { color: c.textSecondary, fontSize: isCompact ? 9 : 10 }]}>
                ...
                <View style={styles.headerRight}>
                    ...
                    <View style={[styles.iconCircle, { backgroundColor: c.cardSecondary, width: 24, height: 24 }]}>
            ...
            {isCompact ? (
                <View style={[styles.compactMain, { flex: 1, justifyContent: 'center' }]}>
                    ...
                    />
                    <View style={[styles.compactGradeBadge, { backgroundColor: `${gradeColor}11` }]}>
                        <Text style={[styles.compactGradeText, { color: gradeColor }]}>{grade.toUpperCase()}</Text>
                    </View>
                </View>
            ) : (
                <View style={styles.gaugeContainer}>
                    ...
                    <View style={[styles.scoreBox, { bottom: svgH - hubY + 15 }]} pointerEvents="none">
                        <Text style={[styles.scoreNum, { color: gradeColor, fontSize: 32, lineHeight: 36 }]}>{totalScore}</Text>
                        <Text style={[styles.scoreOut, { color: c.textSecondary, fontSize: 14 }]}>/ 100</Text>
                    </View>
                </View>
            )}
        </TouchableOpacity>
