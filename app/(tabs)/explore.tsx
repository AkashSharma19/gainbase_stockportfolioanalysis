...
export default function ExploreScreen() {
  ...
  const clearRecentSearches = usePortfolioStore(
  ...
  const filteredTickers = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      ...
      const aChange =
        aYesterday !== 0 ? ((aCurrent - aYesterday) / aYesterday) * 100 : 0;

      const bCurrent = b['Current Value'] || 0;
      const bYesterday = b['Yesterday Close'] || bCurrent;
      const bChange =
        bYesterday !== 0 ? ((bCurrent - bYesterday) / bYesterday) * 100 : 0;
  ...
  const topMovers = useMemo(() => {
    const sorted = [...tickers]
      .filter(
        (t) =>
          t && t.Tickers && t['Company Name'] && t['Asset Type'] !== 'Index',
      )
      .map((t) => {
                  ...
                  >
                    <TouchableOpacity
                      style={[
                        ...
                        filterAssetType === null && styles.filterChipActive,
                      ]}
                      onPress={() => {
                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    ...
                    {uniqueAssetTypes.map((at) => (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          filterAssetType === at && styles.filterChipActive,
                        ]}
                        onPress={() => {
                          Haptics.impactAsync(
            ...
            ListHeaderComponent={
              <>
                <MarketRibbon
                  ...
                  isVisible={
                    !searchQuery && !filterAssetType && !isSearchFocused
                  }
