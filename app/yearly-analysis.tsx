...
export default function YearlyAnalysisScreen() {
    ...
    const barData = useMemo(() => {
        const sorted = [...yearlyAnalysis].reverse(); // Chronological order
        const limited = sorted.slice(-6); // Only show last 6 years
        return limited.map((item: any, index: number) => {
            const prevItem = index > 0 ? limited[index - 1] : null;
            const hasIncreased = !prevItem || item.averageMonthlyInvestment >= prevItem.averageMonthlyInvestment;
            const barColor = hasIncreased ? '#4CAF50' : '#F44336';

            return {
                value: item.averageMonthlyInvestment,
                ...
                topLabelComponent: () => (
                    <Text style={{ color: currColors.textSecondary, fontSize: 8, marginBottom: 4 }}>
                        {isPrivacyMode ? '****' : (item.averageMonthlyInvestment / 1000).toFixed(0) + 'k'}
                    </Text>
                ),
            };
        });
    }, [yearlyAnalysis, isPrivacyMode, currColors.textSecondary]);

    useEffect(() => {
        // Auto-scroll only if we have more than 6 years (though currently limited to 6)
        if (barData.length > 6) {
            setTimeout(() => {
    ...
    const maximumValue = useMemo(() => {
        ...
        const max = Math.max(...yearlyAnalysis.map(item => item.averageMonthlyInvestment));
        ...
        <SafeAreaView style={[styles.safeArea, { backgroundColor: currColors.background }]} edges={['top', 'left', 'right']}>
            <View style={[styles.container, { backgroundColor: currColors.background }]}>
                <View style={[styles.header, { backgroundColor: currColors.background }]}>
                    ...
                    <Text style={[styles.headerTitle, { color: currColors.text }]}>Yearly Trend</Text>
                    <View style={{ width: 40 }} />
                </View>
                <ScrollView
                ...
                >
                    <View style={styles.chartSection}>
                        ...
                        >
                            <View style={styles.chartTitleRow}>
                                <Text style={[styles.chartTitle, { color: currColors.textSecondary }]}>AVG. MONTHLY TREND</Text>
                            ...
                            <View style={styles.chartWrapper}>
                                {barData.length > 0 ? (
                                    <BarChart
                                        ...
                                        xAxisThickness={0}
                                        yAxisThickness={0}
                                        yAxisTextStyle={{ color: currColors.textSecondary, fontSize: 10 }}
                                        xAxisLabelTextStyle={{ color: currColors.textSecondary, fontSize: 10 }}
                                        ...
                                        renderTooltip={(item: any) => (
                                            <View style={[styles.tooltip, { backgroundColor: currColors.card, marginBottom: -10 }]}>
                                                <Text style={{ color: '#FFF', fontSize: 12, fontWeight: '600' }}>
                                                    {showCurrencySymbol ? '₹' : ''}{formatCurrency(item.value)}
                                                </Text>
                                            </View>
                                        )}
                                    />
                                ) : (
                                    <Text style={{ color: currColors.textSecondary }}>No data available</Text>
                                )}
                            </View>
                        </LinearGradient>
                    </View>
                    ...
                    <View style={[styles.accordionContainer, { backgroundColor: currColors.card, borderColor: currColors.border }]}>
                        <Text style={[styles.innerSectionTitle, { color: currColors.textSecondary }]}>YEARLY BREAKDOWN</Text>
                                    ...
                                    {isExpanded && (
                                        <View style={[styles.accordionBody, { backgroundColor: currColors.card }]}>
                                            ...
                                            <View style={styles.assetsGrid}>
                                                {item.assetDistribution.map((asset: any, i: number) => (
